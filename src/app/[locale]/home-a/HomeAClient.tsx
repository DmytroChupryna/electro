  ...
  return (
    <section className="relative min-h-screen flex items-center bg-slate-900 overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute top-0 right-0 w-1/2 h-1/2 bg-orange-600/10 blur-[100px] rounded-full" />
      ...
      <div className="container mx-auto px-4 lg:px-8 pt-32 pb-20 relative z-10">
        <div className="max-w-4xl">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-orange-500/10 border border-orange-500/20 mb-8 animate-fade-in">
            ...
            <span className="text-sm font-medium text-orange-400">{t('tagline')}</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.slice(0, 3).map((service, index) => {
            ...
            return (
              <div
                key={service.key}
                className="group p-8 rounded-2xl bg-slate-900 border border-slate-800 hover:border-orange-500/50 transition-all duration-300 hover:-translate-y-1"
                style={{ animationDelay: `${index * 100}ms` }}
              >
                <div className="w-14 h-14 rounded-xl bg-orange-500/10 flex items-center justify-center mb-6 group-hover:bg-orange-600 transition-colors">
...
function PortfolioSection() {
  ...
  const projects = [
    ...
    {
      key: 'logistics',
      image: '/projects/antwerp-prison/team-work.png',
      category: 'industrial',
    },
  ];
            ...
            <div key={project.key} className="group cursor-pointer">
              <div className="relative overflow-hidden rounded-2xl mb-4 aspect-[4/3]">
                ...
                <div className="absolute bottom-4 left-4 right-4">
                  <span className="inline-block px-3 py-1 rounded-full bg-orange-500/80 text-white text-xs font-medium mb-2">
...
function LocationsSection() {
  ...
  const countries = [
    { key: 'poland', coat: '/coats/poland.png' },
    { key: 'belgium', coat: '/coats/belgium.png' },
    { key: 'eu', coat: '/coats/eu-flag.png' },
  ];
      ...
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-3xl mx-auto">
          {countries.map((country) => (
            <div key={country.key} className="p-6 rounded-2xl bg-slate-950 border border-slate-800 text-center hover:border-orange-500/50 transition-colors">
              ...
              <h3 className="text-lg font-bold text-white">{t(country.key)}</h3>
            </div>
          ))}
        </div>
      </div>
  ...
  const partners = [
    {
      ...
      url: 'https://eeg.be/en/',
      stats: { employees: '850+', experience: '90 years', projects: '400+/year' },
    },
    {
      key: 'netlink',
      logo: '/partners/netlink-logo.png',
      url: 'https://www.netlink.be/',
      stats: { focus: 'Networks', location: 'Gent, BE', specialty: 'A-Z' },
    },
  ];
      ...
      <div className="container mx-auto px-4 lg:px-8">
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-orange-500/10 border border-orange-500/20 mb-6">
            <span className="text-sm font-medium text-orange-400">{t('trustedPartnerships')}</span>
          </div>
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">{t('title')}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {partners.map((partner) => (
            <a
              ...
              className="group p-8 rounded-2xl bg-gradient-to-br from-slate-900 to-slate-900/50 border border-slate-800 hover:border-orange-500/50 transition-all duration-300 hover:-translate-y-1"
            >
              {/* Logo */}
              <div className="mb-8 h-28 flex items-center">
                <div className="bg-slate-500 border border-slate-400 rounded-xl p-6">
                  <img
                    src={partner.logo}
                    alt={`${t(`${partner.key}.name`)} logo`}
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <span className="text-sm text-orange-400">{t('officialPartner')}</span>
                </div>
              ...
              <div className="flex flex-wrap gap-3">
                {Object.entries(partner.stats).map(([key, value]) => (
                  <span
                    key={key}
                    className="px-3 py-1.5 rounded-full bg-slate-800/50 text-sm text-slate-300 border border-slate-700"
                  >
                    {value}
                  </span>
                ))}
  ...
  return (
    <section className="py-24 bg-gradient-to-r from-orange-600 to-orange-700">
      <div className="container mx-auto px-4 lg:px-8 text-center">
        <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">{t('title')}</h2>
        <p className="text-orange-100 mb-8 max-w-xl mx-auto">{t('subtitle')}</p>
        <div className="flex flex-wrap justify-center gap-4">
          <Link
            href="/contact"
            className="inline-flex items-center gap-2 px-8 py-4 rounded-full bg-white text-orange-600 font-semibold hover:bg-orange-50 transition-colors"
          >
            {t('button')}
            <ArrowRight className="w-5 h-5" />
          </Link>
          <a
            ...
            className="inline-flex items-center gap-2 px-8 py-4 rounded-full bg-orange-500 text-white font-semibold hover:bg-orange-400 transition-colors"
          >
...
// Main Page Component
export default function HomeAClient() {
  return (

        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="relative hidden lg:block">
            <div className="w-full aspect-square bg-slate-200 border-4 border-slate-900 relative overflow-hidden">
              ...
              <img
                src="/hero-main.webp"
                alt="Industrial electrical installation work"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0">
            {services.slice(0, 3).map((service, index) => {
              ...
              return (
                <div
                  key={service.key}
                  className={`p-8 bg-white hover:bg-amber-50 transition-colors border-b-4 border-slate-900 ${
                    index < 2 ? 'lg:border-r-4' : ''
                  } ${index === 0 ? 'md:border-r-4' : ''}`}
                >
              ...
              return (
                <div
                  ...
                  className={`p-8 bg-white hover:bg-amber-50 transition-colors ${
                    index === 0 ? 'md:border-r-4 border-slate-900' : ''
                  }`}
                >
                  <div className="w-16 h-16 bg-slate-900 flex items-center justify-center mb-6">
                    <Icon className="w-8 h-8 text-amber-500" />
...
function PortfolioSection() {
  ...
  const projects = [
    {
      key: 'prison',
      image: '/projects/antwerp-prison/conduit-installation.png',
    },
    {
      key: 'office',
      image: '/projects/antwerp-prison/data-cabling.png',
    },
    {
      key: 'logistics',
      image: '/projects/antwerp-prison/cable-routing-2.png',
    },
  ];
...
function LocationsSection() {
  ...
  const countries = [
    { key: 'poland', coat: '/coats/poland.png', dark: false },
    { key: 'belgium', coat: '/coats/belgium.png', dark: true },
    { key: 'eu', coat: '/coats/eu-flag.png', dark: false },
  ];
    ...
    <section className="py-24 bg-amber-500">
      <div className="container mx-auto px-4 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-800 mt-4 font-medium">{t('subtitle')}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-3xl mx-auto">
          {countries.map((country) => (
            <div 
              key={country.key} 
              className={`p-6 border-4 border-slate-900 text-center ${country.dark ? 'bg-slate-900' : 'bg-white'}`}
            >
              <div className="w-14 h-18 mx-auto mb-4">
                <img 
                  src={country.coat} 
                  alt={t(country.key)}
              ...
              <h3 className={`text-xl font-black uppercase ${country.dark ? 'text-amber-500' : 'text-slate-900'}`}>
                {t(country.key)}
              </h3>
            </div>
          ))}
  ...
  const partners = [
    {
      ...
      url: 'https://eeg.be/en/',
      highlight: 'HVAC • Electrical • Medical',
    },
    {
      key: 'netlink',
      logo: '/partners/netlink-logo.png',
      url: 'https://www.netlink.be/',
      highlight: 'Cabling • WiFi • Servers',
    },
  ];

  return (
    <section className="py-24 bg-white border-y-4 border-slate-900">
      <div className="container mx-auto px-4 lg:px-8">
        <div className="text-center mb-16">
          <div className="inline-block bg-amber-500 text-slate-900 font-black px-4 py-2 mb-6 uppercase tracking-widest text-sm">
            {t('title')}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-4xl mx-auto">
          {partners.map((partner) => (
            <a
              ...
              className="group p-8 bg-slate-100 border-4 border-slate-900 hover:bg-amber-50 transition-colors"
            >
              {/* Logo */}
              <div className="mb-8 h-28 flex items-center">
                <div className="bg-slate-400 p-6 border-2 border-slate-900">
                  <img
                    src={partner.logo}
              ...
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight group-hover:text-amber-600 transition-colors">
                  {t(`${partner.key}.name`)}
                </h3>
                <ArrowRight className="w-6 h-6 text-slate-400 group-hover:text-amber-600 group-hover:translate-x-1 transition-all" />
              </div>
              <p className="text-slate-600 font-medium mb-4">{t(`${partner.key}.description`)}</p>
              <div className="inline-block bg-slate-900 text-amber-500 font-bold px-4 py-2 text-sm uppercase tracking-wide">
                {partner.highlight}
              </div>
            </a>
          ))}
...
// Main Page
export default function HomeBClient() {
  return (
